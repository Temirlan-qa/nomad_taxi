...
import 'package:nomad_taxi/src/features/profile/data/mappers/promocode_response_dto_mapper.dart';
...
class ProfileRepositoryImpl implements IProfileRepository {
  ...
  Future<Either<DomainException, PromocodeResponse>> activatePromocode(
    ...
    try {
      final result = await _profileImpl.activatePromocode(request);
      return result.fold(
        (error) => Left(error),
        (dto) {
          final PromocodeResponse entity =
              PromocodeResponseDtoMapper().map(dto);
          return Right(entity);
        },
      );
    } catch (e) {
